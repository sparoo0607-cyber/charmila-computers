...
export function ProductGridCard({ product, compareProduct }: ProductGridCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Product Image */}
      <Link href={`/products/${product.slug}`} className="relative h-48 w-full mb-5 flex items-center justify-center overflow-hidden rounded-2xl bg-[#040812] group-hover:bg-[#080C14] transition-colors border border-transparent group-hover:border-[#1E2D45]/50">
      ...
      <div className="flex flex-col flex-1 relative z-10">
        <span className="text-[9px] font-black uppercase tracking-[0.3em] text-[#2563EB] mb-2 group-hover:text-[#00D4FF] transition-colors line-clamp-1">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[#1E2D45]/50 gap-3">
          <ProductCardActions product={compareProduct} />
          <Link href={`/products/${product.slug}`} className="flex-1">
            <button className="w-full relative overflow-hidden rounded-xl p-[1px] group/btn">
              ...
              <div className="relative w-full bg-[#040812] group-hover/btn:bg-transparent transition-colors duration-300 rounded-xl px-4 py-2 flex items-center justify-center">
                <span className="text-[10px] font-black text-white uppercase tracking-widest group-hover/btn:text-black transition-colors duration-300">
                  Details
                </span>
              </div>
            </button>
